...
class ReportBuilder:
    ...
    def __init__(self, movies):
        ...
        self.movies_by_year = {}
        for movie in movies:
            if movie.start_year not in self.movies_by_year:
                self.movies_by_year[movie.start_year] = []
            self.movies_by_year[movie.start_year].append(movie)
    ...
    def build_year_report(self, year):
        ...
        year_movies = self.movies_by_year.get(year, [])
    ...
    def build_genre_report(self, genre):
        ...
        genre_movies = self.movies_by_genre.get(genre, [])
    ...
    def build_votes_report(self, year):
        ...
        year_movies = [m for m in self.movies_by_year.get(year, []) if m.num_votes is not None]
